...
void URotationCameraComponent::RotatePitch(const float Value, const bool RotateOffset)
{
	if (RotateOffset)
	{
		const float NewPitchOffset = DesiredRotationOffset.Pitch - LastDeltaTime * Value * RotateSpeed;
		SetDesiredPitchOffset(NewPitchOffset);
	}
	else
	{
		const float NewPitch = DesiredRotation.Pitch - LastDeltaTime * Value * RotateSpeed;
...
void URotationCameraComponent::RotateYaw(const float Value, const bool RotateOffset)
{
	if (RotateOffset)
	{
		const float NewYawOffset = DesiredRotationOffset.Yaw - LastDeltaTime * Value * RotateSpeed;
		SetDesiredYawOffset(NewYawOffset);
	}
	else
	{
		const float NewYaw = DesiredRotation.Yaw - LastDeltaTime * Value * RotateSpeed;
...
void URotationCameraComponent::RotateRoll(const float Value, const bool RotateOffset)
{
	if (RotateOffset)
	{
		const float NewRollOffset = DesiredRotationOffset.Roll - LastDeltaTime * Value * RotateSpeed;
		SetDesiredRollOffset(NewRollOffset);
	}
	else
	{
		const float NewRoll = DesiredRotation.Roll - LastDeltaTime * Value * RotateSpeed;		
